...
void aesEncrypt(uint8_t* in, uint8_t* out, AESSmall256*& aes) {
    if (aes == nullptr) {
        return;
    }
...
void aesSetKey(const uint8_t* key_bytes, size_t key_len, AESSmall256*& aes) {
    delete aes;
    aes = nullptr;
    if (key_len != 0) {
        aes = new AESSmall256();
...
static void aes_ccm_auth_start(size_t M, size_t L, const uint8_t* nonce, const uint8_t* aad, size_t aad_len, size_t plain_len, uint8_t* x, AESSmall256*& aes) {
    ...
    aesEncrypt(b, x, aes); /* X_1 = E(K, B_0) */
    if (!aad_len)
        return;
    ...
    aesEncrypt(aad_buf, x, aes); /* X_2 = E(K, X_1 XOR B_1) */
    if (aad_len > AES_BLOCK_SIZE - 2) {
        xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
        /* X_3 = E(K, X_2 XOR B_2) */
        aesEncrypt(&aad_buf[AES_BLOCK_SIZE], x, aes);
    }
}
static void aes_ccm_auth(const uint8_t* data, size_t len, uint8_t* x, AESSmall256*& aes) {
...
static void aes_ccm_encr(size_t L, const uint8_t* in, size_t len, uint8_t* out, uint8_t* a, AESSmall256*& aes) {
    size_t last = len % AES_BLOCK_SIZE;
    size_t i;
...
static void aes_ccm_encr_auth(size_t M, const uint8_t* x, uint8_t* a, uint8_t* auth, AESSmall256*& aes) {
    size_t i;
    uint8_t tmp[AES_BLOCK_SIZE];
    /* U = T XOR S_0; S_0 = E(K, A_0) */
    WPA_PUT_BE16(&a[AES_BLOCK_SIZE - 2], 0);
    aesEncrypt(a, tmp, aes);
    for (i = 0; i < M; i++)
        auth[i] = x[i] ^ tmp[i];
}
static void aes_ccm_decr_auth(size_t M, uint8_t* a, const uint8_t* auth, uint8_t* t, AESSmall256*& aes) {
    size_t i;
    uint8_t tmp[AES_BLOCK_SIZE];
    /* U = T XOR S_0; S_0 = E(K, A_0) */
    WPA_PUT_BE16(&a[AES_BLOCK_SIZE - 2], 0);
...
int aes_ccm_ae(const uint8_t* key, size_t key_len, const uint8_t* nonce, size_t M, const uint8_t* plain, size_t plain_len, const uint8_t* aad, size_t aad_len, uint8_t* crypt, uint8_t* auth, AESSmall256*& aes) {
    ...
    if (aad_len > 30 || M > AES_BLOCK_SIZE)
        return -1;
    aesSetKey(key, key_len, aes);
    aes_ccm_auth_start(M, L, nonce, aad, aad_len, plain_len, x, aes);
    aes_ccm_auth(plain, plain_len, x, aes);
    /* Encryption */
    aes_ccm_encr_start(L, nonce, a);
    aes_ccm_encr(L, plain, plain_len, crypt, a, aes);
    aes_ccm_encr_auth(M, x, a, auth, aes);
    return 0;
}
/* AES-CCM with fixed L=2 and aad_len <= 30 assumption */
bool aes_ccm_ad(const uint8_t* key, size_t key_len, const uint8_t* nonce, size_t M, const uint8_t* crypt, size_t crypt_len, const uint8_t* aad, size_t aad_len, const uint8_t* auth, uint8_t* plain, AESSmall256*& aes) {
    ...
    /* Decryption */
    aes_ccm_encr_start(L, nonce, a);
    aes_ccm_decr_auth(M, a, auth, t, aes);
    /* plaintext = msg XOR (S_1 | S_2 | ... | S_n) */
    aes_ccm_encr(L, crypt, crypt_len, plain, a, aes);
    aes_ccm_auth_start(M, L, nonce, aad, aad_len, crypt_len, x, aes);
    aes_ccm_auth(plain, crypt_len, x, aes);
